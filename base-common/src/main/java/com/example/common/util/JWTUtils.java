...
import static com.example.common.util.Consts.TOKEN_PRE;
...
public class JWTUtils {
    ...
    public static UserToken extractToken(String claimsJws) {
        if (claimsJws.startsWith(TOKEN_PRE)) {
            claimsJws = claimsJws.replaceFirst(TOKEN_PRE, "");
        }
        Claims claims;
        try {
            ...
            log.warn("Error in validating token：{}", e.getMessage());
            throw new CommonException(401, "Jwt expired", e, 401);
        } catch (Exception e) {
            log.warn("Error in validating token：{}", e.getMessage());
            throw new CommonException(401, "Please login", e, 401);
        }
        ...
        builder.id(claims.getId())
                ...
                .version((Integer) claims.get(UserToken.VERSION_STR));


        Object code = claims.get(UserToken.CODE_STR);
        if (code != null) {
